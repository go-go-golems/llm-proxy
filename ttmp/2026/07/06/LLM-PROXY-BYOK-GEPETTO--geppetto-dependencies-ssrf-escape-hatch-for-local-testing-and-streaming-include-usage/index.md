---
Title: 'Geppetto dependencies: SSRF escape hatch for local testing and streaming include_usage'
Ticket: LLM-PROXY-BYOK-GEPETTO
Status: active
Topics:
    - byok
    - geppetto
    - llm-proxy
    - security
    - inference
DocType: index
Intent: long-term
Owners: []
RelatedFiles:
    - Path: geppetto/pkg/security/outbound_url.go
      Note: ValidateOutboundURL + OutboundURLOptions — the SSRF guard primitive
    - Path: llm-proxy/pkg/byok/integration_test.go
      Note: In-process fake engine — canonical CI test seam to document
    - Path: llm-proxy/pkg/runtime/chat_service.go
      Note: Streaming goroutine owning result.Usage — include_usage plumbing point
ExternalSources: []
Summary: ""
LastUpdated: 2026-07-06T11:09:01.635703608-04:00
WhatFor: ""
WhenToUse: ""
---




# Geppetto dependencies: SSRF escape hatch for local testing and streaming include_usage

## Overview

<!-- Provide a brief overview of the ticket, its goals, and current status -->

## Key Links

- **Related Files**: See frontmatter RelatedFiles field
- **External Sources**: See frontmatter ExternalSources field

## Status

Current status: **active**

## Topics

- byok
- geppetto
- llm-proxy
- security
- inference

## Tasks

See [tasks.md](./tasks.md) for the current task list.

## Changelog

See [changelog.md](./changelog.md) for recent changes and decisions.

## Structure

- design/ - Architecture and design documents
- reference/ - Prompt packs, API contracts, context summaries
- playbooks/ - Command sequences and test procedures
- scripts/ - Temporary code and tooling
- various/ - Working notes and research
- archive/ - Deprecated or reference-only artifacts
