...
- [x] Meter circuit breaker: track consecutive ledger-write failures in `meter.Recorder` (or a health struct); trip a 503 data plane after a configurable threshold (`--byok-meter-failure-threshold`); reset on successful write; tests simulating persistent store failure
- [x] Meter circuit breaker: decide default threshold and whether to distinguish transient `SQLITE_BUSY` from persistent (disk-full) failures
