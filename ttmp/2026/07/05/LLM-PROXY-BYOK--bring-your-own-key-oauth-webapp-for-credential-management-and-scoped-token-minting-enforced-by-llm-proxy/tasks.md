...
- [ ] Phase 0: define DB schema (users, credentials, tokens, scopes, usage_ledger)
- [ ] Phase 1: bearer-token auth middleware in llm-proxy with SQLite token store and model allowlist
- [ ] Phase 1: CLI token minting (llm-proxy-server token mint) for testing without UI
- [ ] Phase 2: encrypted per-user credential vault and per-request profile/credential resolution
